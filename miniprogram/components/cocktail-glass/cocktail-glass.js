...
Component({
  ...
  data: {
    liquidColor: PALETTE.calm.main,
    liquidLight: PALETTE.calm.light,
    liquidDark: PALETTE.calm.dark,
    glowGradient: 'radial-gradient(ellipse at center, rgba(78, 205, 196, 0.5) 0%, transparent 70%)',
    waveDuration: 2.4,
    waveDurationB: 3.2,
    waveDurationC: 1.7,
    waveAmplitude: 6,
  ...
  observers: {
    'direction, intensity, fluctuation, isAnalyzing'() {
      this.applyVisualParams()
    },
  },
  ...
  methods: {
    resolveDirection(dir) {
      ...
      if (key === 'bright' || key === 'calm' || key === 'deep') {
        return key
      }
      return 'calm'
    },
    ...
    applyVisualParams() {
      const direction = this.resolveDirection(this.properties.direction)
      const intensity = normIntensity(this.properties.intensity)
      const fluctuation = normFluctuation(this.properties.fluctuation)
      const analyzing = !!this.properties.isAnalyzing
      const c = PALETTE[direction]

      const waveBase = 3.2 - (fluctuation / 100) * 2.1
      const waveDuration = clamp(waveBase, 1.05, 3.25)
      const waveAmplitude = 4 + (fluctuation / 100) * 10

      const glowOpacity = 0.12 + (intensity / 100) * 0.62
      const smokeOpacity = analyzing ? 0.22 + (intensity / 200) * 0.45 : 0
      const smokeDuration = 4.2 + (1 - fluctuation / 100) * 2.4
      const smokeDuration2 = smokeDuration * 1.18

      const glowGradient = `radial-gradient(ellipse at center, ${c.glow} 0%, transparent 72%)`
      ...
      this.setData({
        ...
        glowGradient,
        waveDuration,
        waveDurationB: waveDuration * 1.32,
        waveDurationC: waveDuration * 0.72,
        waveAmplitude,
        glowOpacity,
        smokeOpacity,
        smokeDuration,
    ...
    tickBubbles() {
      ...
      for (let i = 0; i < this._bubbles.length; i += 1) {
        ...
        grd.addColorStop(0.45, hexToRgba(c.light, 0.42 * b.a))
        grd.addColorStop(1, hexToRgba(c.main, 0.08 * b.a))
