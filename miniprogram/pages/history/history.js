...
function normalizeItem(row) {
  ...
  const id = row._id != null ? row._id : row.id != null ? row.id : row.cocktailId
...
function normalizeHistoryResult(result) {
  ...
  if (Array.isArray(result)) {
    ...
    if (Array.isArray(result.list)) arr = result.list
...
Page({
  ...
  fetchHistory() {
    ...
    return wx.cloud
      ...
      .then(res => {
        const list = normalizeHistoryResult(res.result).map(it => ({
          ...it,
          displayTime: formatTime(new Date(getItemTime(it))),
