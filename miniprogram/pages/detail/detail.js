...
function applyPayload(page, payload, cocktailId) {
  ...
  const hasEmotionQuote = !!emotionText;
  ...
  let savedAtTs =
    payload.createdAt != null
      ...
      : payload.savedAt != null
        ? payload.savedAt
        : cocktail.savedAt;
  if (typeof savedAtTs === 'object' && savedAtTs != null && typeof savedAtTs.getTime === 'function') {
    savedAtTs = savedAtTs.getTime();
  }
  const ts =
    typeof savedAtTs === 'number'
      ? savedAtTs
      : parseInt(savedAtTs, 10);
  const savedAtText = !Number.isNaN(ts) && ts > 0 ? util.formatTime(ts) : '';

  const concRaw = recipe.concentration;
  const concNum = typeof concRaw === 'number' ? concRaw : parseFloat(concRaw);
  const concentrationPercent = Number.isNaN(concNum)
    ? 0
    : Math.min(100, Math.max(0, Math.round(concNum)));
  const concentrationDisplay = `${concentrationPercent}%`;
  const feelingText = safeStr(recipe.feeling);

  const summary = safeStr(cocktail.summary || recipe.summary);
  page._shareSummary = summary;
  page._cocktailId = cocktailId || safeStr(payload._id || payload.id || cocktail.id || cocktail._id);
...
Page({
  data: {
    ...
    emotionData: null,
    emotionText: '',
    hasEmotionQuote: false,
    feelingText: '',
    concentrationPercent: 0,
    concentrationDisplay: '0%',
  ...
  onLoad(options) {
    ...
    const scrollPadTop = navBarHeight + 16;
